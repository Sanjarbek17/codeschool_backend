...
from .serializers import (
    HomeworkProgressSerializer,
    HomeworkProgressListSerializer,
    HomeworkProgressCreateUpdateSerializer,
    TaskProgressSerializer,
    TaskProgressListSerializer,
    TaskProgressCreateUpdateSerializer,
    StudentProgressSummarySerializer,
    TeacherProgressDashboardSerializer,
    LessonProgressAnalyticsSerializer,
    HomeworkAnalyticsSerializer,
    StudentAnalyticsSerializer,
    ProgressComparisonSerializer,
    LearningPathSerializer,
    ProgressInsightsSerializer,
)
...
class HomeworkProgressViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            return (
                HomeworkProgress.objects.all()
                .select_related("student__user", "homework__lesson")
            ...
            return (
                HomeworkProgress.objects.filter(homework__lesson__teachers=user.teacher)
                .select_related("student__user", "homework__lesson")
                .prefetch_related("homework__lesson__teachers")
            )
            ...
            return (
                HomeworkProgress.objects.filter(student=user.student)
                .select_related("student__user", "homework__lesson")
                .prefetch_related("homework__lesson__teachers")
            )
    ...
    def summary(self, request):
        ...
        homework_completion_rate = (
            (completed_homework / total_homework * 100) if total_homework > 0 else 0
        )
        ...
        summary_data = {
            ...
            "recent_homework": HomeworkProgressListSerializer(
                recent_homework, many=True
            ).data,
        }
    ...
    def teacher_dashboard(self, request):
        ...
        for lesson in lessons:
            ...
            avg_homework_completion = (
                (homework_completion / lesson_progress.count() * 100)
                if lesson_progress.count() > 0
                else 0
            )
            avg_task_completion = (
                (task_completion / lesson_tasks.count() * 100)
                if lesson_tasks.count() > 0
                else 0
            )
            ...
            active_students = (
                lesson_progress.filter(last_attempt_at__gte=week_ago)
                .values("student")
                .distinct()
                .count()
            )
            ...
            # Recent activity
            recent_activity = lesson_progress.aggregate(recent=Max("last_attempt_at"))[
                "recent"
            ]
        ...
        return Response(
            {"teacher": request.user.get_full_name(), "dashboard": serializer.data}
        )
    ...
    def lesson_analytics(self, request):
        ...
        analytics_data = {
            ...
            "overview": {
                ...
                "total_tasks": total_tasks,
                "homework_completion_rate": (
                    (completed_homework / lesson_progress.count() * 100)
                    if lesson_progress.count() > 0
                    else 0
                ),
                "task_completion_rate": (
                    (completed_tasks / lesson_tasks.count() * 100)
                    if lesson_tasks.count() > 0
                    else 0
                ),
            },
...
class TaskProgressViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            # Students see only their own progress
            return (
                TaskProgress.objects.filter(student=user.student)
                .select_related(
                    "student__user", "task__homework__lesson", "last_submission"
                )
                .prefetch_related("task__homework__lesson__teachers")
    ...
    def _update_homework_progress(self, homework, student):
        ...
        solved_tasks = TaskProgress.objects.filter(
            task__homework=homework, student=student, is_solved=True
        ).count()
    ...
    def homework_analytics(self, request):
        ...
        # Task breakdown
        task_breakdown = task_progress.values("task__title", "task__id").annotate(
            total_students=Count("student", distinct=True),
        ...
        student_progress = homework_progress.values(
            ...
            total_tasks=Count(
                "student__task_progress",
                filter=Q(student__task_progress__task__homework=homework),
            ),
