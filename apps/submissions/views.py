...
from .serializers import (
    HomeworkSubmissionSerializer, HomeworkSubmissionListSerializer,
    HomeworkSubmissionCreateSerializer, HomeworkSubmissionUpdateSerializer,
    TestCaseSerializer, TestCaseListSerializer, TestCaseCreateSerializer,
    TestCaseStudentSerializer, SubmissionStatisticsSerializer,
    StudentProgressSerializer, TaskAnalyticsSerializer
)
...
class HomeworkSubmissionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            return HomeworkSubmission.objects.all().select_related(
                'student__user', 'task__homework__lesson'
    ...
    def statistics(self, request):
        ...
        stats_data = submissions.values(
            ...
            successful_submissions=Count('id', filter=Q(passed_tests__gte=F('total_tests'))),
    ...
    def task_analytics(self, request):
        ...
        if hasattr(request.user, 'teacher') and not request.user.is_superuser:
            if not task.homework.lesson.teachers.filter(id=request.user.teacher.id).exists():
                return Response(
        ...
        completed_students = submissions.filter(
            passed_tests__gte=F('total_tests')
        ...
        for student_id in lesson_students:
            ...
            if student_submissions.exists():
                best_submission = student_submissions.order_by('-passed_tests', 'submitted_at').first()
        ...
        student_attempts = submissions.values('student').annotate(
            attempts=Count('id')
        ).values_list('attempts', flat=True)
        average_attempts = sum(student_attempts) / len(student_attempts) if student_attempts else 0
...
class TestCaseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            return TestCase.objects.filter(
                task__homework__lesson__in=enrolled_lessons,
                hidden=False
            ).select_related('task__homework__lesson')
